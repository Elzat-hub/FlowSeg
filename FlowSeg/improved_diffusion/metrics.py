...
def HD_metric(x_np, gt_np):
    predict_points = np.argwhere(x_np == 1)
    target_points = np.argwhere(gt_np == 1)

    H, W = x_np.shape 

    if len(predict_points) == 0 and len(target_points) == 0:
        hd = 0.0 
    elif len(predict_points) == 0 or len(target_points) == 0:
        hd = np.sqrt(H**2 + W**2) 
    else:
        hd1 = directed_hausdorff(predict_points, target_points)[0] 
        hd2 = directed_hausdorff(target_points, predict_points)[0] 
        hd = max(hd1, hd2)
